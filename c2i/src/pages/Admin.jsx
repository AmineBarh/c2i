...
const Admin = () => {
  ...
  // ⚡ Bolt: Consolidated 4 O(N) array traversals (filter) into a single O(N) reduce inside useMemo
  // to prevent unnecessary recalculations on unrelated renders (like activeSection or sidebarOpen changes).
  const { filteredProjects, totalProjects, iotProjects, webProjects, automationProjects } = useMemo(() => {
    return projects.reduce((acc, project) => {
      ...
      else if (project.type === "web") acc.webProjects++;
      else if (project.type === "automation") acc.automationProjects++;
