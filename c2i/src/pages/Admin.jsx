...
const Admin = () => {
  ...
  // ⚡ Bolt: Memoize filtered projects to prevent O(N) recalculations on unrelated state changes (like sidebar toggles)
  ...
  // ⚡ Bolt: Combine multiple O(N) filter passes into a single O(N) reduce pass for project category counts
  const { iotProjects, webProjects, automationProjects, totalProjects } = useMemo(() => {
    ...
    return { ...counts, totalProjects: projects.length };
  }, [projects]);
