...
const Admin = () => {
  ...
  // ⚡ Bolt: Consolidated 3 sequential O(N) array filter passes into a single O(N) loop and memoized the result to prevent recalculation on unrelated state changes (like sidebar toggles).
  const { totalProjects, iotProjects, webProjects, automationProjects } =
    useMemo(() => {
      let iot = 0;
      let web = 0;
      let automation = 0;
