...
const Admin = () => {
  ...
  // ⚡ Bolt: Wrap filteredProjects in useMemo to prevent O(N) array filtering on unrelated re-renders (like opening the sidebar)
  ...
  // ⚡ Bolt: Memoize derived project counts
  const iotProjects = useMemo(() => {
    return projects.filter((p) => p.type === "iot").length;
  }, [projects]);

  const webProjects = useMemo(() => {
    return projects.filter((p) => p.type === "web").length;
  }, [projects]);

  const automationProjects = useMemo(() => {
    return projects.filter((p) => p.type === "automation").length;
  }, [projects]);
