...
const Admin = () => {
  ...
  // ⚡ Bolt: Memoize filtered projects to prevent redundant evaluations on every render
  const filteredProjects = React.useMemo(() => projects.filter(
    (project) =>
  ...
  // ⚡ Bolt: Combine multiple O(N) filter passes into a single O(N) reduce pass, and memoize
  const { totalProjects, iotProjects, webProjects, automationProjects } = React.useMemo(() => {
