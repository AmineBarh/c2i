...
const TrainingDashboard = ({ trainings = [], handleCreateTraining, handleUpdateTraining, handleDeleteTraining }) => {
  ...
  // ⚡ Bolt: Memoize filtered trainings and extract search term conversion to prevent O(N) re-evaluation on irrelevant state changes
  const filteredTrainings = useMemo(() => {
    const lowerSearchTerm = searchTerm.toLowerCase();
    return trainings.filter((training) => {
      const matchesSearch =
        training.title.toLowerCase().includes(lowerSearchTerm) ||
        training.description.toLowerCase().includes(lowerSearchTerm) ||
        training.instructor.toLowerCase().includes(lowerSearchTerm);
  ...
  // ⚡ Bolt: Compute distinct categories and instructors in a single O(N) pass and memoize stats to prevent recalculation on every render
  const stats = useMemo(() => {
    const uniqueInstructors = new Set();
    const uniqueCategories = new Set();

    trainings.forEach((t) => {
      if (t.instructor) uniqueInstructors.add(t.instructor);
