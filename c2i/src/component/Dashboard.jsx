...
const Dashboard = ({ projects = [], trainings = [], partners = [] }) => {
  ...
  const projectDashboardData = useMemo(() => {
    ...
    // ⚡ Bolt: Replaced O(D*N) loop with O(N+D) Hash Map for project dates
    ...
    projects.forEach((p) => {
      ...
      else if (p.type === "automation") autoCount++;
  ...
  const trainingDashboardData = useMemo(() => {
    ...
    // ⚡ Bolt: Replaced O(D*N) loop with O(N+D) Hash Map for training dates and combined operations
