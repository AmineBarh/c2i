...
const Dashboard = ({ projects = [], trainings = [], partners = [] }) => {
  ...
  const projectDashboardData = useMemo(() => {
    ...
    // ⚡ Bolt: Replaced O(Days * Projects) nested loop and multiple O(N) passes
    // with a single O(N) pass and O(Days) map lookup.
    ...
    const dailyActivity = eachDayOfInterval({
      ...
      return {
        ...
        projects: projectsByDate.get(dateKey) || 0,
      };
    });
  ...
  const trainingDashboardData = useMemo(() => {
    ...
    // ⚡ Bolt: Replaced O(Days * Trainings) nested loop with single O(N) pass and O(Days) map lookup
    ...
    trainings.forEach((training) => {
      if (training.createdAt) {
        ...
        const dateKey = format(createdDate, "yyyy MMM dd");
        trainingsByDate.set(dateKey, (trainingsByDate.get(dateKey) || 0) + 1);
      }
    ...
    const dailyActivity = eachDayOfInterval({
      ...
      return {
        date: format(date, "MMM dd"),
        trainings: trainingsByDate.get(dateKey) || 0,
      };
    });
