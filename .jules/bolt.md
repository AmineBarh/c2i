...
## 2024-05-18 - Missing netlify.toml causes deploy failures
**Learning:** A missing `netlify.toml` file in the frontend directory caused Netlify deploy check failures (Header rules, Pages changed, Redirect rules) for the React SPA.
**Action:** Always verify that deployment configuration files exist and are correctly configured to resolve platform-specific CI failures.
